...
static void ui_draw_colorband_handle_tri(float x1, float y1, float halfwidth, float height, bool fill)
{
	...
	GPUBegin(fill ? GL_TRIANGLES : GL_LINE_LOOP);
	glVertex2f(x1 + halfwidth, y1);
	glVertex2f(x1, y1 + height);
	glVertex2f(x1 - halfwidth, y1);
	glEnd();
...
{
	GPUBegin(fill ? GL_QUADS : GL_LINE_LOOP);
	glVertex2f(x1, y1);
	glVertex2f(x1, y2);
	glVertex2f(x2, y2);
	glVertex2f(x2, y1);
...
void ui_draw_but_UNITVEC(uiBut *but, uiWidgetColors *wcol, const rcti *rect)
{
	...
	gpuScale(GPU_MODELVIEW_MATRIX, size, size,  MIN2(size, 1.0f));
...
void ui_draw_but_CURVE(ARegion *ar, uiBut *but, uiWidgetColors *wcol, const rcti *rect)
{
	...
	if (cumap->flag & CUMA_DRAW_SAMPLE) {
		GPUBegin(GL_LINES); /* will draw one of the following 3 lines */
		if (but->a1 == UI_GRAD_H) {
			float tsample[3];
			float hsv[3];
...
void ui_draw_but_TRACKPREVIEW(ARegion *ar, uiBut *but, uiWidgetColors *UNUSED(wcol), const rcti *recti)
{
	...
	if (!ok && scopes->track_preview) {
		...
		if (width > 0 && height > 0) {
			...
			gpuTranslate(GPU_MODELVIEW_MATRIX, rect.xmin + scopes->track_pos[0], rect.ymin + scopes->track_pos[1], 0.f);
