...
{
	float win[3];
	win[0] = ar->winrct.xmin + mval[0] + 0.5f;
	win[1] = ar->winrct.ymin + mval[1] + 0.5f;
	win[2] = depth;

	if (gpuUnProject(
	        win, mats->modelview, mats->projection, (const GLint *)mats->viewport, r_location_world))
	{
		return true;
	}
	return false;
...
static void curve_draw_stroke_3d(const struct bContext *UNUSED(C), ARegion *UNUSED(ar), void *arg)
{
	...
	if (cu->ext2 > 0.0f) {
		...
		gpuMultMatrix(GPU_MODELVIEW_MATRIX, &obedit->obmat[0][0]);
	...
	if (stroke_len > 1) {
...

		{
			...
			cpack(0x0);
			glLineWidth(3.0f);
			GPUDrawArrays(GL_LINE_STRIP, 0, stroke_len);
