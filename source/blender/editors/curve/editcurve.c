...
//} XShape;
//
//static void spline_X_shape(Object *obedit, ListBase *selected_splines)
//{
//	Nurb *nu = selected_splines->first;
//	ListBase *nubase = object_editcurve_get(obedit), *shape_list, *all_segments, self = {NULL, NULL};
//	int useles;
//
//	BLI_addtail(&self, get_selected_splines(nubase, &useles, 0)->first);
//
...
//	ListBase *self_intersections, *intersections;
//	XShape *xshape;
//	float *coord_array, *full_coord_array;
//	self_intersections = (ListBase *)MEM_callocN(sizeof(ListBase), "splineXshape1");
//	full_coord_array = (float *)MEM_callocN(3 * nu->pntsu * nu->resolu, "splineXshape2");
//
//	for (LinkData *segment = all_segments->first; segment; segment = segment->next) {
//		coord_array = segment->data;
//		for (int i = 0; i < nu->resolu - 1; i++) {
//			intersections = get_intersections(coord_array + 3 * i, coord_array + 3 * (i + 1), &self);
//			if (intersections->first) {
//				xshape = (XShape *)MEM_callocN(sizeof(XShape), "splineXshape3");
//				xshape->intersections = intersections;
//				BLI_addtail(self_intersections, intersections);
//			}
//		}
//	}
//
//	//get_intersections(0, 0, nubase);
//
...
static int trim_curve_exec(bContext *C, wmOperator *op)
{
	...
	//spline_X_shape(obedit, selected_splines);
