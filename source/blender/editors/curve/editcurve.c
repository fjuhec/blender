			...
			for (segment = ((ListBase *)spline->data)->first; segment; segment = segment->next, k++) {
				...
				for (a = 0; a < nu->resolu; a++)
				{
					...
					if (result == 1)
					{
						...
						l2 = len_v3(helper);
						copy_v3_v3(helper, &full_coord_array[i * 3]);
						sub_v3_v3(helper, &segment_coord_array[(a + 1) * 3]);
						l3 = len_v3(helper);
						copy_v3_v3(helper, &full_coord_array[(i + 1) * 3]);
						sub_v3_v3(helper, &segment_coord_array[(a + 1) * 3]);
						l4 = len_v3(helper);

						if (!(l1 < PRECISION || l2 < PRECISION || l3 < PRECISION || l4 < PRECISION)) {
							xshape = (XShape *)MEM_callocN(sizeof(XShape), "splineXshape6");
							xshape->intersections = vi;
							xshape->order = i / nu->resolu;
							copy_v3_v3(helper, vi);
							sub_v3_v3(helper, &full_coord_array[i * 3]);
...
static int trim_curve_exec(bContext *C, wmOperator *op)
{
	...
	else
	{
		...
		else if (len_high > 0 && len_low > 0)
		{
			...
			copy_v3_v3(new_spl1->bezt[0].vec[1], ((XShape *)((LinkData *)high.first)->data)->intersections);
...
static int curve_fillet_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
	...
	if (event->val == KM_PRESS && has_numinput && handleNumInput(C, &opdata->num_input[opdata->value_mode], event)) {
		...
		switch (event->type) {
			...
			case MOUSEMOVE:
				if (!has_numinput) {
					float val = (curr_mouse[0] - prev_mouse[0]) * (curr_mouse[0] - prev_mouse[0])	\
	...
	ofs += BLI_snprintf(str + ofs, sizeof(str) - ofs, IFACE_("Chamfer distance: %.2f"), distance);
	ED_area_headerprint(CTX_wm_area(C), str);
