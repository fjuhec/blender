...
static void gp_draw_strokes(
        ARegion *ar, bGPdata *gpd, const bGPDframe *gpf, int offsx, int offsy, int winx, int winy, int dflag,
...
static void gp_draw_onionskins(
        ARegion *ar, bGPdata *gpd, const bGPDlayer *gpl, const bGPDframe *gpf, int offsx, int offsy, int winx, int winy,
        int UNUSED(cfra), int dflag, bool debug, const float diff_mat[4][4])
{
	...
	if (gpl->gstep > 0) {
		...
		for (bGPDframe *gf = gpf->prev; gf; gf = gf->prev) {
			...
			if ((gpf->framenum - gf->framenum) <= gpl->gstep) {
				...
				gp_draw_strokes(ar, gpd, gf, offsx, offsy, winx, winy, dflag, debug, gpl->thickness, 1.0f, color,
		...
		if (gpf->prev) {
			color[3] = (alpha / 7);
			gp_draw_strokes(ar, gpd, gpf->prev, offsx, offsy, winx, winy, dflag, debug, gpl->thickness, 1.0f, color,
			                true, gpl->flag & GP_LAYER_GHOST_PREVCOL, diff_mat);
		}
	}
	else {
	...
	if (gpl->gstep_next > 0) {
		...
		if (gpf->next) {
			color[3] = (alpha / 4);
			gp_draw_strokes(ar, gpd, gpf->next, offsx, offsy, winx, winy, dflag, debug, gpl->thickness, 1.0f, color,
			                true, gpl->flag & GP_LAYER_GHOST_NEXTCOL, diff_mat);
		}
	}
	else {
...
void ED_gp_draw_interpolation(const bContext *C, tGPDinterpolate *tgpi, const int type)
{
	...
	for (tgpil = tgpi->ilayers.first; tgpil; tgpil = tgpil->next) {
		...
		if (tgpil->interFrame) {
			gp_draw_strokes(ar, tgpi->gpd, tgpil->interFrame, offsx, offsy, winx, winy, dflag, false,
...
static void gp_draw_data_layers(
        const bGPDbrush *brush, float alpha, Object *ob, ARegion *ar, bGPdata *gpd,
...
#undef GP_DRAWFLAG_APPLY
		    ...
		    ((dflag & GP_DRAWDATA_NO_ONIONS) == 0 || (gpl->flag & GP_LAYER_GHOST_ALWAYS))) 
		{
			 ...
			 */
			gp_draw_onionskins(ar, gpd, gpl, gpf, offsx, offsy, winx, winy, cfra, dflag, debug, diff_mat);
		}

		/* draw the strokes already in active frame */
		gp_draw_strokes(ar, gpd, gpf, offsx, offsy, winx, winy, dflag, debug, gpl->thickness,
        ...
        int offsx, int offsy, int winx, int winy, int cfra, int dflag)
{
	...
	/* draw! */
	gp_draw_data_layers(brush, alpha, ob, ar, gpd, offsx, offsy, winx, winy, cfra, dflag);
...
static void gp_draw_data_all(Scene *scene, ARegion *ar, bGPdata *gpd, int offsx, int offsy, int winx, int winy,
                             int cfra, int dflag, const char spacetype)
{
	...
	if (scene) {
		...
		if (gpd_source) {
			if (brush != NULL) {
				gp_draw_data(brush, ts->gp_sculpt.alpha, NULL, ar, gpd_source,
	...
	if (gpd_source == NULL || (gpd_source && gpd_source != gpd)) {
		if (brush != NULL) {
			gp_draw_data(brush, ts->gp_sculpt.alpha, NULL, ar, gpd,
			             offsx, offsy, winx, winy, cfra, dflag);
		}
	}
}
...
void ED_gpencil_draw_2dimage(const bContext *C)
{
	...
	/* draw it! */
	gp_draw_data_all(scene, ar, gpd, offsx, offsy, sizex, sizey, CFRA, dflag, sa->spacetype);
}
...
void ED_gpencil_draw_view2d(const bContext *C, bool onlyv2d)
{
	...
	gp_draw_data_all(scene, ar, gpd, 0, 0, ar->winx, ar->winy, CFRA, dflag, sa->spacetype);
...
void ED_gpencil_draw_view3d(wmWindowManager *wm, Scene *scene, View3D *v3d, ARegion *ar, bool only3d)
{
	...
	/* draw it! */
	gp_draw_data_all(scene, ar, gpd, offsx, offsy, winx, winy, CFRA, dflag, v3d->spacetype);	
}
...
void ED_gpencil_draw_view3d_object(wmWindowManager *wm, Scene *scene, Object *ob, View3D *v3d, ARegion *ar, bool only3d)
{
	...
	* deal with the camera border, otherwise map the coords to the camera border. */
	...
	if (brush != NULL) {
		gp_draw_data(brush, ts->gp_sculpt.alpha, ob, ar, gpd,
			offsx, offsy, winx, winy, CFRA, dflag);
	}
}

void ED_gpencil_draw_ex(Scene *scene, ARegion *ar, bGPdata *gpd, int winx, int winy, const int cfra, const char spacetype)
{
	int dflag = GP_DRAWDATA_NOSTATUS | GP_DRAWDATA_ONLYV2D;

	gp_draw_data_all(scene, ar, gpd, 0, 0, winx, winy, cfra, dflag, spacetype);
}
