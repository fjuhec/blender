...
void drawcircball(int mode, const float cent[3], float rad, const float tmat[4][4])
{
	...
	GPUDrawArrays(mode, 0, CIRCLE_RESOL);
	glDisableClientState(GL_VERTEX_ARRAY);
}
...
static void drawcentercircle(View3D *v3d, RegionView3D *rv3d, const float co[3], int selstate, bool special_color)
{
	...
	GPUDrawArrays(GL_POLYGON, 0, CIRCLE_RESOL);

	/* 2. draw outline */
	glLineWidth(1);
	UI_ThemeColorShadeAlpha(TH_WIRE, 0, -30);
	GPUDrawArrays(GL_LINE_LOOP, 0, CIRCLE_RESOL);
...
static void drawcube_size(float size)
{
	...
	GPUDrawRangeElements(GL_LINES, 0, 7, 24, GL_UNSIGNED_BYTE, indices);
	glDisableClientState(GL_VERTEX_ARRAY);
}
                     ...
                     const char dt, const short dflag, const unsigned char ob_wire_col[4], const bool is_obact)
{
	...
	if (is_view) {
		...
		if (la->mode & LA_SQUARE) {
			...
			/* draw the pyramid */
			GPUDrawElements(GL_LINE_STRIP, 12, GL_UNSIGNED_BYTE, indices);

			/* deactivate vertex arrays after drawing */
			glDisableClientState(GL_VERTEX_ARRAY);

			gpuTranslate(GPU_MODELVIEW_MATRIX, 0.0f, 0.0f, x);
                       ...
                       const short dflag, const unsigned char ob_wire_col[4])
{
	...
	if (!is_stereo3d_cameras) {
		...
		if (is_selection_camera_stereo) {
			...
			gpuPushMatrix(GPU_MODELVIEW_MATRIX);
			gpuLoadMatrix(GPU_MODELVIEW_MATRIX, &rv3d->viewmat[0][0]);
			BKE_camera_multiview_model_matrix(&scene->r, ob, is_left ? STEREO_LEFT_NAME : STEREO_RIGHT_NAME, obmat);
			gpuMultMatrix(GPU_MODELVIEW_MATRIX, &obmat[0][0]);
	...
	for (int i = 0; i < 2; i++) {
		if (i == 0) GPUBegin(GL_LINE_LOOP);
		else if (i == 1 && is_active) GPUBegin(GL_TRIANGLES);
		else break;
		...
		switch (dl->type) {
			case DL_SEGM:
				...
				for (parts = 0; parts < dl->parts; parts++)
					GPUDrawArrays(GL_LINE_STRIP, parts * dl->nr, dl->nr);
			...
			case DL_POLY:
				...
				for (parts = 0; parts < dl->parts; parts++)
					GPUDrawArrays(GL_LINE_LOOP, parts * dl->nr, dl->nr);
			...
			case DL_SURF:
				...
				for (int nr = 0; nr < dl->nr; nr++) {
					...
					if (dl->flag & DL_CYCL_V) GPUBegin(GL_LINE_LOOP);
				...
				if (dl->flag & DL_CYCL_V)
					GPUDrawArrays(GL_LINE_LOOP, 0, dl->parts);
				else
					GPUDrawArrays(GL_LINE_STRIP, 0, dl->parts);
#endif
				}
				break;
			...
			case DL_INDEX3:
				...
				GPUDrawElements(GL_TRIANGLES, 3 * dl->parts, GL_UNSIGNED_INT, dl->index);
				break;

			case DL_INDEX4:
				glVertexPointer(3, GL_FLOAT, 0, dl->verts);
				GPUDrawElements(GL_QUADS, 4 * dl->parts, GL_UNSIGNED_INT, dl->index);
				break;
		}
	}
		...
		switch (dl->type) {
			case DL_SEGM:
				if (ob->type == OB_SURF) {
					...
					// glVertexPointer(3, GL_FLOAT, 0, dl->verts);
					// GPUDrawArrays(GL_LINE_STRIP, 0, dl->nr);
					GPUBegin(GL_LINE_STRIP);
			...
			case DL_POLY:
				if (ob->type == OB_SURF) {
					...
					/* for some reason GPUDrawArrays crashes here in half of the platforms (not osx) */
					//glVertexPointer(3, GL_FLOAT, 0, dl->verts);
					//GPUDrawArrays(GL_LINE_LOOP, 0, dl->nr);
			...
			case DL_SURF:
				...
				if (dl->index) {
					...
					GPUDrawElements(GL_QUADS, 4 * dl->totindex, GL_UNSIGNED_INT, dl->index);
...
static void draw_particle_arrays(int draw_as, int totpoint, int ob_dt, int select)
{
	...
	switch (draw_as) {
		...
		case PART_DRAW_CROSS:
			GPUDrawArrays(GL_LINES, 0, 6 * totpoint);
			break;
		case PART_DRAW_LINE:
			GPUDrawArrays(GL_LINES, 0, 2 * totpoint);
			break;
		case PART_DRAW_BB:
			...
			GPUDrawArrays(GL_QUADS, 0, 4 * totpoint);
			break;
		default:
			GPUDrawArrays(GL_POINTS, 0, totpoint);
			break;
	}
}
...
/* 6. */
	...
	if (draw_as == PART_DRAW_PATH) {
		...
		for (a = 0, pa = psys->particles; a < totpart; a++, pa++) {
			...
			if (path->segments > 0) {
				...
				GPUDrawArrays(GL_LINE_STRIP, 0, path->segments + 1);
			}
		}
			...
			if (part->draw & PART_DRAW_GUIDE_HAIRS) {
				...
				for (a = 0, pa = psys->particles; a < totpart; a++, pa++) {
					if (pa->totkey > 1) {
						...
						GPUDrawArrays(GL_LINE_STRIP, 0, pa->totkey);
					}
				}
	...
	if (pdd && pdd->vedata) {
		...
		GPUDrawArrays(GL_LINES, 0, 2 * totve);
	}
	...
	if (pset->selectmode != SCE_SELECT_PATH) {
		...
		if (pset->selectmode == SCE_SELECT_POINT) {
			...
			for (i = 0, point = edit->points; i < totpoint; i++, point++) {
				...
				GPUDrawArrays(GL_POINTS, 0, point->totkey);
...
static void draw_box(const float vec[8][3], bool solid)
{
	...
	if (solid) {
		...
		GPUDrawRangeElements(GL_QUADS, 0, 7, 24, GL_UNSIGNED_BYTE, indices);
	}
	else {
