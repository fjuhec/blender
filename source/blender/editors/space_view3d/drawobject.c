...
bool set_wire_colorset(Scene *scene, View3D *v3d, Base *base, unsigned char r_ob_wire_col[4])
{
	...
	if (v3d->wire_color_flag & V3D_WIRE_COLOR_EXCLUDE_SELECT) {
		...
		else if (v3d->wire_color_source == V3D_WIRE_COLOR_SOURCE_MATERIAL) {
			const Material *mat = give_current_material(ob, 1);

			if (mat) {
				r_ob_wire_col[0] = 255 * mat->r;
				r_ob_wire_col[1] = 255 * mat->g;
				r_ob_wire_col[2] = 255 * mat->b;
				r_ob_wire_col[3] = 255;
		...
		if ((is_presel || (v3d->flag & V3D_SELECT_OUTLINE)) && !render_override && ob->type != OB_MESH) {
			if (dt > OB_WIRE && (ob->mode & OB_MODE_EDIT) == 0 && (dflag & DRAW_SCENESET) == 0) {
				if (!(ob->dtx & OB_DRAWWIRE) && (is_presel || (ob->flag & SELECT)) &&
					...
					draw_object_selected_outline(scene, v3d, ar, base, ob_wire_col, is_presel);
				}
			}
		}
